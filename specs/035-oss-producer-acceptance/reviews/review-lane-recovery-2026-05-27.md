...
OpenRouter MiniMax instead of the direct MiniMax provider.
...
| Lane | State | Evidence |
| --- | --- | --- |
| `kimi-coding/kimi-for-coding` | `not_assessed` | Smoke prompt returned `OK-KIMI`, but full and narrowed review prompts timed out after 180 seconds without output. |
...
The default Portolan slice-review MiniMax lane is now:
...
Review infrastructure is partially restored:

- MiniMax slice review is restored through the approved OpenRouter lane.
- GLM slice review is restored.
- Kimi remains degraded for review prompts and should be split into smaller
  packets or recorded as `not_assessed` when it times out.
